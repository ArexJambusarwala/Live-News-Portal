...
2035.39
        ...
        <div id="content" class="container">
            <center><h1 style="margin-top: 5px; margin-bottom: 15px; font-family: fira Sans; font-weight: 700;  font-style: normal;">Sports</h1></center>
              <div class="row">
                  <div id="sports_1">
    ...
    <script>
                    ...
                    echo $row["COUNT(*)"]?>;i++)
        {
                document.getElementById("heading"+i).innerHTML="<?php
                    ...
                    $query="SELECT `Title` FROM `sports` WHERE `ID`=1";
                    $query=mysqli_real_escape_string($db,$query);
                    $result=mysqli_query($db,$query);   
                    $row=mysqli_fetch_assoc($result);
                    echo $row["Title"];?>";
             document.getElementById("image"+i).src="<?php
                    $db = mysqli_connect('localhost', 'livenews_portal', 'creatives@2k18', 'livenews_news');
                    $query="SELECT `Image` FROM `sports` WHERE `ID`=1";
                    $query=mysqli_real_escape_string($db,$query);
                    $result=mysqli_query($db,$query);   
                    $row=mysqli_fetch_assoc($result);
                    echo $row["Image"];?>";
                document.getElementById("brief"+i).innerHTML="<?php 
                    $db = mysqli_connect('localhost', 'livenews_portal', 'creatives@2k18', 'livenews_news');
                    $query="SELECT `Brief` FROM `sports` WHERE `ID`=1";
                    $query=mysqli_real_escape_string($db,$query);
                    $result=mysqli_query($db,$query);
                    $row=mysqli_fetch_assoc($result);
                    echo $row["Brief"];
                ...
                document.getElementById("date"+i).innerHTML="<?php 
                    $db = mysqli_connect('localhost', 'livenews_portal', 'creatives@2k18', 'livenews_news');
                    $query="SELECT `Date` FROM `sports` WHERE `ID`=1";
                    $query=mysqli_real_escape_string($db,$query);
                    $result=mysqli_query($db,$query);
                    $row=mysqli_fetch_assoc($result);
                    echo $row["Date"];            
